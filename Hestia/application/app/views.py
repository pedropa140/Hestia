...
def chart(request, ticker, chartname):
    try:
        ...
        if chartname == 'book_to_share':
            ax.plot(start_date, book_to_share_value, label='book_to_share')
#         book.xlabel('Date')
            ax.set_ylabel('Book to Share Value Ratio')
            ax.set_title(f'{ticker.upper()}: Book to Share Value Over Time')
        elif chartname == 'current_ratio':
            ax.plot(start_date, current_ratio, label='current_ratio')
            ax.set_ylabel('Current Ratio')
            ax.set_title(f'{ticker.upper()}: Current Ratio Over Time')
        elif chartname == 'debt_ratio':
            ax.plot(start_date, debt_ratio, label='debt_ratio')
            ax.set_ylabel('Debt Ratio')
            ax.set_title(f'{ticker.upper()}: Debt Ratio Over Time')
        elif chartname == 'dividend_yield':
            ax.plot(start_date, dividend_yield, label='dividend_yield_ratio')
            ax.set_ylabel('Dividend Yield Ratio')
            ax.set_title(f'{ticker.upper()}: Dividend Yield Ratio Over Time')
        elif chartname == 'earnings_per_share':
            ax.plot(start_date, earnings_per_share, label='dividend_yield_ratio')
            ax.set_ylabel('Earnings Per Share')
            ax.set_title(f'{ticker.upper()}: Earnings Per Share Over Time')
        elif chartname == 'stock_prices':
            ax.plot(start_date, start_open, label='start_open')
            ax.plot(start_date, start_close, label='start_close')
            ax.plot(start_date, start_high, label='start_high')
            ax.plot(start_date, end_open, label='end_open')
            ax.plot(start_date, end_close, label='end_close')
            ax.plot(start_date, end_high, label='end_high')
            
            ax.set_title(f'{ticker.upper()}: Earnings Per Share Over Time')
            ax.set_ylabel('Stocks Per Share')
        ax.set_xlabel('Date')
        ax.legend()
        # ax.set_xticks(rotation=45, ha='right')
        # ax.gcf().set_size_inches(20, 10)
        stock_buffer = io.BytesIO()
        fig.savefig(stock_buffer, format='png')
        stock_buffer.seek(0)
        plt.close(fig)
        # return JsonResponse({'path': f"../stockdata/stockpictures/debt_ratio_picture/{ticker}.png"})
        return HttpResponse(stock_buffer, content_type='image/png')
